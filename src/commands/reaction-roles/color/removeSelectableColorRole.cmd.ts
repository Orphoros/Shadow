    ...
    if (await isUserAuthorized(interaction, interaction.guild)) {
      ...
      if (roleOption !== null) {
        SelectableColorRoleOption.deleteOne({
          ...
          Remove the old menu that hold this color role and display a it again to make this change affective!`, EmbedMessageType.Success, 'Could not send interaction message to user');
        }).catch((e) => {
          sendCrashResponse(interaction, `Could not delete color role <@&${role?.id}> from the remote database!`, e);
        });
      } else {
        sendResponse(interaction, `Cannot remove color role <@&${role?.id}>! This role is not added to this color selection menu! It might be in an other color`, EmbedMessageType.Warning, 'Could not send interaction message to user');
      }
    } else {
